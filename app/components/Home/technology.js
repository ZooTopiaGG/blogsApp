...
export default class TechnologyRoute extends Component {
  constructor(props) {
    super(props);
    this.state = {
      result: [],
          ...
          this.state.result.map((val, index) => {
            return (
              <TouchableOpacity
                ...
                key={ index } 
                onPress={() => { 
                  navigate('Details', { id: val.id, headerTitle: val.title }) // 子页面接收传过去的参数值
                }}
                title={ val.title }
                >
                <View style={{ paddingLeft: px2dp(30), paddingRight: px2dp(30), paddingTop: px2dp(30), paddingBottom: px2dp(30), borderBottomWidth: 1, borderColor:'#e1e1e1', borderStyle: 'solid' }}>
                  <View style={{ flexDirection: 'column',marginBottom:px2dp(30)}}>
                    <Text numberOfLines={2} style={{ color: '#444', fontSize: 16 }}>{ val.title }</Text>
                    <Text style={{marginTop:px2dp(10), fontSize: 12, color: '#999'}}>邓鹏 / 文章 / {val.type==1 ? '转载': '原创'} / 2017-11-12  阅读量：{val.viewcount} </Text>
                  </View>
